# SE Prices Printed CLI Agent Guide

This directory is a generated `se-prices-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
se-prices-pp-cli doctor --json
se-prices-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
se-prices-pp-cli which "<capability>" --json
se-prices-pp-cli <command> --help
```
...
```bash
se-prices-pp-cli <command> --agent
```
...
```bash
se-prices-pp-cli <command> --help
se-prices-pp-cli <command> --dry-run --agent
```
