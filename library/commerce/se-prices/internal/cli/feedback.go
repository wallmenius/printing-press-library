...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".se-prices-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SE_PRICES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SE_PRICES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "se-prices-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.se-prices-pp-cli/feedback.jsonl.
When ` + "`SE_PRICES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SE_PRICES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  se-prices-pp-cli feedback list
  se-prices-pp-cli feedback list --limit 5
  se-prices-pp-cli feedback list --json`,
