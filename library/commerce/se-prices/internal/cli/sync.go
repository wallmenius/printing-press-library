...
Exit codes & warnings:
  ...
  se-prices-pp-cli sync

  # Sync specific resources only
  se-prices-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  se-prices-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  se-prices-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  se-prices-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  se-prices-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("se-prices-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/se-prices-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "pricerunner":
		return db.UpsertPricerunner(data)
	case "prisjakt":
		return db.UpsertPrisjakt(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		// pricerunner.deals is hosted on a different base than the spec root
		// (root is prisjakt.nu, pricerunner uses base_url override).
		// The sync helper hands this to client.Get verbatim, so use the full URL.
		"pricerunner": "https://www.pricerunner.se/deals",
	}
