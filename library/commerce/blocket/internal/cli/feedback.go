...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".blocket-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BLOCKET_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BLOCKET_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "blocket-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.blocket-pp-cli/feedback.jsonl.
When ` + "`BLOCKET_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BLOCKET_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  blocket-pp-cli feedback list
  blocket-pp-cli feedback list --limit 5
  blocket-pp-cli feedback list --json`,
