# Blocket Printed CLI Agent Guide

This directory is a generated `blocket-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
blocket-pp-cli doctor --json
blocket-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
blocket-pp-cli which "<capability>" --json
blocket-pp-cli <command> --help
```
...
```bash
blocket-pp-cli <command> --help
blocket-pp-cli <command> --dry-run --agent
```
